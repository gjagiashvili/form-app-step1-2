...
function Card2(props) {
    ...
    const [advancedIsValid, setAdvancesIsValid] = useState(true)
    const [proIsValid, setProIsValid] = useState(true)
    ...
    const goNext = () => {
        if (Plan === '') {
            errorMessageValidator(!planIsValid, 'Please select atleast one')
            setShowError(true)
        }
        else { 
            props.onClick();
    ...
    return (
        <>
            ...
            <div className="card-2" id="card-2" >
                ...
                <button className={`step-2-Plan ${Plan === 'Arcade' ? 'step-2-Plan-Selected' : ''}`} onClick={() => selectPlans("Arcade")}>
                    {Icon1}
                    <h3>Arcade</h3>
                    <p>10$/mo</p>
                    
                </button>
                <button className={`step-2-Plan ${Plan === 'Advanced' ? 'step-2-Plan-Selected' : ''}`} onClick={() => selectPlans("Advanced")}>
                    {Icon2}
                    <h3>Advanced</h3>
                    <p>12$/mo</p>
                   
                </button>
                <button className={`step-2-Plan ${Plan === 'Pro' ? 'step-2-Plan-Selected' : ''}`} onClick={() => selectPlans("Pro")}>
                    {Icon3}
                    <h3>Pro</h3>
                    <p>15$/mo</p>                    
                ...
                {showError && (   <div className='Error1'><span className='Error1'>You must select atleast 1 plan</span> </div>)} <br></br>

                <button className="goBackButton" onClick={props.GoBack}>Go Back</button>
                <button className="submit-button" onClick={goNext}>Next Step</button>
